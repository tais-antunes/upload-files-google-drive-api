        ...
        try {
            ...
            const filePromises = fileDataArray.map(async (fileData) => {
                ...
                const response = await driveService.files.create({
                    ...
                    media: media,
                    fields: 'id',  // Solicitando que o ID seja incluído na resposta
                });

                const driveFileId = response.data.id;  // Obtendo o ID do Google Drive

                return { id: driveFileId, name: fileData.originalname };
            });
            ...
            res.json(uploadedFiles);  // Enviando a resposta com os IDs do Google Drive
        } catch (err) {
            console.error('Erro no upload dos arquivos:', err);
